...
FILTER_VALUE = 1.0
RANK_TOP_UPPER_LIMIT = 10


def write_excel_file(header, max_order_indexes, array2d):
    wb = openpyxl.Workbook()
    sheet = wb.active
    sheet.title = 'top_result'
    sheet = wb['top_result']
    for i in range(len(header)):
        sheet.cell(row=1, column=i+1).value = header[i]
    for row in range(len(array2d)):
        for col in range(len(max_order_indexes)):
            sheet.cell(row=row+2, column=col+1).value = array2d[row][max_order_indexes[col]]
    wb.save('../output/top_result.xlsx')


def write_csv_file(header: List[str], max_order_indexes: List[int], array2d: List[List[str]]):
    with open('../output/top_result.csv', 'w', newline='') as csvfile:
...
def write_file(pids: List[str], array2d: List[List[str]]):
    ...
    write_csv_file(header, max_order_indexes, array2d)
    write_excel_file(header, max_order_indexes, array2d)
...
def analyze_top_log_lines(lines: List[str], pids: List[str], time_mem_array2d: List[List[str]]):
...
def analyze_top_log(file_path: str):
    ...
    with open(file_path, 'r') as f:
        lines = f.readlines()
        analyze_top_log_lines(lines, pids, time_mem_array2d)
